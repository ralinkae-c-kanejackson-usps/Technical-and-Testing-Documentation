...
## Context
Business objective: Increase participation volume, attract new participants, and increase USPS revenue.

Implementation under test: MuleSoft sends a CRID parameter to an API hosted or supported by a Databricks App. The service queries saldev.db_rpt.mailing_promotions_crid_high_account_t3 and returns a JSON response containing only records whose crid equals the MuleSoft-provided CRID. The implementation must prevent unfiltered results and data leakage between CRIDs.
